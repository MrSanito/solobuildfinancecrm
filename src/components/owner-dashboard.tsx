...
import {
  ...
  LineChartIcon, ChevronRight, Plus,
} from "lucide-react";
...
export default function OwnerDashboard() {
  ...
  return (
    <div className="w-full bg-[#f8f9fc] p-3 md:p-5">

      {/* ════════════════════════════════════════════════════
          3-COLUMN LAYOUT (matches Figma)
          Col 1: 1, 2, 3
          Col 2: 4, 5, 6, 7
          Col 3: 8, 9, 10, 11
      ════════════════════════════════════════════════════ */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-3 items-start">
        ...
        <div className="flex flex-col gap-3">
          ...
          <SectionCard num="2" icon={Users} title="Receivables Pressure" iconBg="bg-orange-500">
            ...
            {/* Two sub-columns: clients list + donut chart */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-xs text-gray-400 font-medium mb-1.5">Top 5 Delayed Clients</p>
                <div className="flex flex-col gap-1.5">
                  {["Alpha Corp","Beta Mfg","Gamma Ltd","Delta Inc","Epsilon Co"].map((name, i) => (
                    <div key={name} className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold text-red-500 shrink-0">₹{[3.2,2.8,2.1,1.9,1.4][i]}L</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-xs text-gray-400 font-medium mb-1.5">Aging Snapshot</p>
                <div className="flex justify-center mb-2">
                  <ResponsiveContainer width={90} height={90}>
                    <PieChart>
                      ...
                      <Tooltip formatter={(v: any) => [`${v}%`, ""]} contentStyle={{ borderRadius: 8, fontSize: 10 }} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
          ...
          <SectionCard num="3" icon={ShoppingCart} title="Payables Pressure" iconBg="bg-red-500">
            ...
            {/* Two sub-columns: critical payments + top vendors */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="flex flex-col gap-1.5">
                  {["Supplier A","Supplier B","Supplier C"].map((s, i) => (
                    <div key={s} className="flex items-center justify-between bg-red-50 rounded-lg px-2 py-1.5">
                      <span className="text-xs text-gray-700">{s}</span>
                      <span className="text-xs font-bold text-red-600">{["₹3.2L","₹2.1L","₹1.8L"][i]}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {["Vendor X","Vendor Y","Vendor Z","Vendor W","Vendor V"].map((v, i) => (
                    <div key={v} className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-gray-600 truncate">{v}</span>
                      </div>
                      <span className="text-xs font-bold text-gray-700 shrink-0">₹{[5.4,4.2,3.8,2.9,2.1][i]}L</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <div key={s.label} className="bg-gray-50 rounded-xl p-2.5 text-center">
                  <p className="text-xs text-gray-400 leading-tight mb-0.5">{s.label}</p>
          ...
          <SectionCard num="6" icon={Zap} title="Expense Intelligence" iconBg="bg-yellow-500">
            ...
            {/* Two sub-cols: bar chart + cost centers */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-xs text-gray-400 mb-1">Spend vs Budget</p>
                <ResponsiveContainer width="100%" height={100}>
                  <BarChart data={spendData} barSize={7}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" vertical={false} />
              ...
              <div>
                <p className="text-xs text-gray-400 mb-1">Top Cost Centers</p>
                ...
                <thead>
                  <tr className="text-xs text-gray-400 font-medium uppercase tracking-wide border-b border-gray-100">
                    {["Priority","Action","Type","Impact (Cash)","Risk Reduction"].map((h) => (
                      <th key={h} className="text-left pb-2 pr-2">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {actions.map((a, i) => (
                    <tr key={i} className="hover:bg-gray-50 transition-colors">
                      <td className="py-2 pr-2">
                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${priorityColor[a.priority]}`}>
                          {a.priority}
                        </span>
                      </td>
                      <td className="py-2 pr-2 text-gray-700 font-medium text-xs">{a.action}</td>
                      <td className="py-2 pr-2 text-gray-500 text-xs">{a.type}</td>
                      <td className="py-2 pr-2 font-bold text-gray-800 text-xs">{a.impact}</td>
                      <td className="py-2">
        ...
        <div className="flex flex-col gap-3">
          ...
          <SectionCard num="10" icon={Building2} title="Multi-Entity Overview" iconBg="bg-indigo-500">
            <div className="grid grid-cols-4 text-xs text-gray-400 font-medium uppercase tracking-wide pb-1.5 border-b border-gray-100 mb-1">
              <span>Entity</span>
              <span className="text-right">Revenue (MTD)</span>
              <span className="text-right">Profit (MTD)</span>
              ...
              {entities.map((e) => (
                <div key={e.name} className="grid grid-cols-4 items-center py-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-gray-600 leading-tight truncate">{e.name}</span>
                  </div>
                  <span className="text-xs font-bold text-gray-800 text-right">{e.revenue}</span>
                  <span className="text-xs font-bold text-green-600 text-right">{e.profit}</span>
                  <span className="text-xs font-bold text-blue-600 text-right">{e.cash}</span>
                </div>
              ))}
            </div>
          </SectionCard>
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {[
                ...
                <div key={a.label} className="flex flex-col items-center bg-gray-50 rounded-lg p-2 gap-1">
                  ...
                  <p className="text-xs text-gray-500 text-center leading-tight">{a.label}</p>
                  <p className="text-sm font-bold text-gray-700">{a.pct}</p>
                </div>
              ))}
            </div>
          </SectionCard>
      ...
      {/* ════════ 12. Trends Overview — Full Width Bottom ════════ */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <p className="text-xs text-gray-500 font-medium mb-2 flex items-center flex-wrap gap-x-2 gap-y-1">
              Receivables Aging Trend
