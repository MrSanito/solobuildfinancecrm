    ...
    <div className="min-h-screen bg-[#f8f9fc] p-3 md:p-5 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-3">
        ...
        <SectionCard num="1" icon={Droplets} title="Cash & Liquidity" iconBg="bg-blue-500">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="bg-blue-50 rounded-xl p-2.5">
              <p className="text-[10px] text-blue-500 font-medium">Bank Balance</p>
              <p className="text-sm font-bold text-blue-700">₹18.6L</p>
            </div>
            <div className="bg-green-50 rounded-xl p-2.5">
              <p className="text-[10px] text-green-500 font-medium">Cash In Hand</p>
              <p className="text-sm font-bold text-green-700">₹2.1L</p>
            </div>
          </div>
          <p className="text-[10px] text-gray-400 font-medium mb-1.5">Upcoming Outflows</p>
          <div className="flex gap-2">
            {["7 Days","30 Days"].map((d) => (
              <div key={d} className="flex-1 bg-orange-50 rounded-lg p-2 text-center">
                <p className="text-[10px] text-orange-400">{d}</p>
                <p className="text-xs font-bold text-orange-600">{d === "7 Days" ? "₹4.2L" : "₹14.8L"}</p>
              </div>
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard num="4" icon={TrendingUp} title="Profit Snapshot" iconBg="bg-green-500">
          ...
          {/* 5. Revenue Quality nested */}
          <div className="border-t border-gray-100 pt-3">
            <p className="text-[11px] font-bold text-gray-700 mb-2">5. Revenue Quality</p>
            ...
            <p className="text-[10px] text-gray-400 mb-1">Collection Efficiency Trend</p>
        ...
        <SectionCard num="8" icon={Shield} title="Risk Engine" iconBg="bg-red-500">
          <div className="flex flex-col gap-2.5">
            {riskMetrics.map((r) => (
              <div key={r.label} className="flex items-center justify-between gap-2">
                <p className="text-xs text-gray-600 flex-1 leading-tight">{r.label}</p>
          ...
          <div className="border-t border-gray-100 mt-3 pt-3">
            <p className="text-[11px] font-bold text-gray-700 mb-2">9. Compliance Status</p>
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {[
              ...
              <div key={c.label} className={`${c.bg} rounded-xl p-2.5 text-center`}>
                <p className="text-[9px] text-gray-500 leading-tight mb-0.5">{c.label}</p>
                <p className={`text-xs font-bold ${c.color}`}>{c.value}</p>
              </div>
            ))}
          </div>
          <p className="text-[10px] text-gray-400 font-medium mb-2">Suggested Allocation</p>
          <div className="grid grid-cols-4 gap-1.5">
            {[
              ...
              <div key={a.label} className="flex flex-col items-center bg-gray-50 rounded-lg p-2 gap-1">
                ...
                <p className="text-[9px] text-gray-500 text-center leading-tight">{a.label}</p>
                <p className="text-[10px] font-bold text-gray-700">{a.pct}</p>
              </div>
            ))}
          </div>
        </SectionCard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
        ...
        <SectionCard num="2" icon={Users} title="Receivables Pressure" iconBg="bg-orange-500">
          ...
          <p className="text-[10px] text-gray-400 font-medium mb-2">Aging Snapshot</p>
          ...
          <p className="text-[10px] text-gray-400 font-medium mt-3 mb-1.5">Top 5 Delayed Clients</p>
          <div className="flex flex-col gap-1">
            {["Alpha Corp","Beta Mfg","Gamma Ltd","Delta Inc","Epsilon Co"].map((name, i) => (
              <div key={name} className="flex items-center justify-between py-1 border-b border-gray-50 last:border-0">
                ...
                <span className="text-[10px] font-bold text-red-500">₹{[3.2,2.8,2.1,1.9,1.4][i]}L</span>
              </div>
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard num="6" icon={Zap} title="Expense Intelligence" iconBg="bg-yellow-500">
          ...
          <p className="text-[10px] text-gray-400 mb-1.5">Spend vs Budget</p>
          ...
          <p className="text-[10px] text-gray-400 mb-1.5 mt-2">Top Cost Centers / Categories</p>
          <div className="flex flex-col gap-1.5">
            {topCostCenters.map((c) => (
              <div key={c.name} className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-gray-700">{c.amount}</span>
              </div>
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard num="10" icon={Building2} title="Multi-Entity Overview" iconBg="bg-indigo-500">
          <div className="mb-1">
            <div className="grid grid-cols-4 text-[9px] text-gray-400 font-medium uppercase tracking-wide pb-1.5 border-b border-gray-100">
              <span>Entity</span>
              ...
              <div key={e.name} className="grid grid-cols-4 items-center py-2">
                <div className="flex items-center gap-1.5">
                  <div className="size-5 rounded-md bg-indigo-50 flex items-center justify-center shrink-0">
                    <Building2 size={9} className="text-indigo-500" />
                  </div>
                  <span className="text-[9px] text-gray-600 leading-tight truncate">{e.name}</span>
                </div>
                <span className="text-[10px] font-bold text-gray-800 text-right">{e.revenue}</span>
                <span className="text-[10px] font-bold text-green-600 text-right">{e.profit}</span>
                <span className="text-[10px] font-bold text-blue-600 text-right">{e.cash}</span>
              </div>
            ))}
          </div>
        </SectionCard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
        ...
        <SectionCard num="3" icon={ShoppingCart} title="Payables Pressure" iconBg="bg-red-500">
          ...
          <p className="text-[10px] text-gray-400 font-medium mb-1.5">Critical Payments (Next 7 Days)</p>
          <div className="flex flex-col gap-1.5 mb-3">
            {["Supplier A","Supplier B","Supplier C"].map((s, i) => (
              <div key={s} className="flex items-center justify-between bg-red-50 rounded-lg px-2.5 py-1.5">
                ...
                <span className="text-[10px] font-bold text-red-600">{["₹3.2L","₹2.1L","₹1.8L"][i]}</span>
              </div>
            ))}
          </div>
          <p className="text-[10px] text-gray-400 font-medium mb-1.5">Top Vendors by Exposure</p>
          <div className="flex flex-col gap-1">
            {["Vendor X","Vendor Y","Vendor Z","Vendor W","Vendor V"].map((v, i) => (
              <div key={v} className="flex items-center justify-between py-1 border-b border-gray-50 last:border-0">
                ...
                <span className="text-[10px] font-bold text-gray-700">₹{[5.4,4.2,3.8,2.9,2.1][i]}L</span>
              </div>
            ))}
          </div>
        </SectionCard>
        ...
        <div className="md:col-span-2">
          <SectionCard num="7" icon={ListChecks} title="Actions Panel (Top Priorities)" iconBg="bg-blue-600">
                  ...
                  {actions.map((a, i) => (
                    <tr key={i} className="hover:bg-gray-50 transition-colors">
                      <td className="py-2.5 pr-3">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold ${priorityColor[a.priority]}`}>
                          {a.priority}
                        </span>
                      </td>
                      <td className="py-2.5 pr-3 text-gray-700 font-medium text-xs">{a.action}</td>
                      <td className="py-2.5 pr-3 text-gray-500 text-xs">{a.type}</td>
                      <td className="py-2.5 pr-3 font-bold text-gray-800">{a.impact}</td>
                      <td className="py-2.5">
                        <span className={`text-xs font-bold ${riskBadge[a.risk]}`}>● {a.risk}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <a href="#" className="flex items-center justify-center gap-1 mt-3 text-xs text-blue-600 py-2 border border-gray-100 rounded-lg hover:bg-blue-50 transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-[11px] font-bold text-gray-700 tracking-wide">12. Trends Overview</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Cash Flow Trend */}
          <div>
            <p className="text-[10px] text-gray-500 font-medium mb-1.5 flex items-center gap-2">
